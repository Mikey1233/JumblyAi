...
function ChatListRow({ chatId }: { chatId: string }) {
  ...
  const row = (message?: Message) => (
    ...
    >
      <UserAvatar
        name={message?.user.name || session?.user.username}
        image={message?.user.image || session?.user.imageUrl}
      />
      <div className="flex-1">
        <p className="font-bold">
          {!message && "New Chat"}
          {message &&
            [message?.user.name || session?.user.username].toString().split(" ")[0]}
        </p>
    ...
    <div>
      {loading && (
        <div className="flex p-5 items-center space-x-2">
          <Skeleton className="h-12 w-12 items-center space-x-2" />
